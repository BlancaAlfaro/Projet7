...
def previous_applications(num_rows = None, nan_as_category = True,data_folder="../data"):
    ...
    prev.loc[prev['APP_CREDIT_PERC']==np.inf,'APP_CREDIT_PERC']=np.nan
...
def installments_payments(num_rows = None, nan_as_category = True,data_folder="../data"):
    ...
    #ADDITION : If AMT_INSTALMENT is 0, PAYMENT_PERC should be nan
    ins.loc[ins['AMT_INSTALMENT']==0,'PAYMENT_PERC']=np.nan
...
def main(debug = False,data_folder="../data"):
    ...
    df_features=df.dropna(axis='columns',how='any')
    df=pd.concat([df_features,df['TARGET']],axis=1)
    del df_features
    gc.collect()
